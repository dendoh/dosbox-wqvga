...
static void OutputString(Bitu,Bitu,const char *,Bit32u,Bit32u,SDL_Surface *);
...
void GFX_EndUpdate( const Bit16u *changedLines ) {
	...
	int acc_ticks;
	acc_ticks = GetTicks() - last_ticks;
	frame_count++;
	if (acc_ticks > 1000) {
	   fps_dirty = true;
	   fps = frame_count/(float)acc_ticks*1000.0;
	   frame_count=0;
	   last_ticks = GetTicks();
	}
	if (!sdl.updating)
	...
	case SCREEN_SURFACE_DINGUX:
		...
		if (fps_dirty) {
		   sprintf(string_fps,"fps:%.2f",fps);
		   OutputString(0,0,string_fps,0xffffffff,0,sdl.surface); //Show FPS at (0,0)
		   fps_dirty = false;
		}
		SDL_Flip(sdl.surface);
		break;
